...
public class Brain {
    ...
    public void sendAccountMail( Worker worker ) {
        ...
        try {
            ...
            gMailer.setMessage("Ihr Benutzername ist: " + worker.getUsername()
                    + System.lineSeparator()
                    + "Ihr Passwort lautet: " + worker.getPasswort()
                    + System.lineSeparator()
                    + System.lineSeparator()
                    + System.lineSeparator()
                    + "Bitte ändern sie schnellst möglich ihr Passwort zu ihrem eigenen.");
            gMailer.sendMail();
        } catch (Exception ex) {
    ...
    public void sendTaskMail( Worker worker, String subject, String description, int zeitaufwand ) {
        ...
        try {
            ...
            gMailer.setMessage(description + ""
                    ...
                    + "Zeitaufwand geplant für diese Aufabe sind : " + zeitaufwand + "Minuten");
            gMailer.sendMail();
        } catch (Exception ex) {
            Logger.getLogger(GUINewTask.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void SendInvoiceMail( Task task, String[][] data ) {
        Worker worker = new Worker(task.getCreator());
        String adress = worker.getEmail();
        try {
            GMailer gMailer = new GMailer();
            gMailer.setToEmailAddress(adress);
            gMailer.setSubject("Abrechnung für die Aufgabe : #" + task.getiD_Task());
            gMailer.setMessage("Im Anhang finden sie die Abrechnung der Aufgabe #" + task.getiD_Task() + "."
                    + System.lineSeparator());
            gMailer.sendMailWithAttachment(data);
        } catch (Exception ex) {
            Logger.getLogger(GUINewTask.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public String[] getAllUsersIDs( String[] filter ) {
        this.fillWorkerArray();
        List<String> allIds = new LinkedList<>();
        List<String> usernameFilter = Arrays.asList(filter);
        for (Worker w : allWorkers) {
            if (usernameFilter.contains(w.getUsername())) {
                allIds.add(w.getName() + ", " + w.getFirstName() + " (" + w.getUsername() + ")");
            }
        }
